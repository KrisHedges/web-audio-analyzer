...
function guessSourceInfo(name: string): {
  ...
  const sourceType = 'Device'; // Default
  ...
  if (manufacturer) {
    ...
    const brandIdx = parts.findIndex(
      (p) => p.toLowerCase() === manufacturer!.toLowerCase(),
    );
    ...
    if (/SE-50/i.test(name)) {
      manufacturer = 'Roland';
      model = 'SE-50';
    } else if (/PCM/i.test(name)) {
      manufacturer = 'Lexicon';
    } else if (/SPX/i.test(name)) {
      manufacturer = 'Yamaha';
    }
  }
